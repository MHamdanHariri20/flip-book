        ...
        <div class="col-12 d-flex align-items-stretch">
          <div class="card w-100">
            <div class="card-body p-4">
              ...
              <form action="{{ route('delete5') }}" method="POST">
              @csrf
              @method('DELETE')
                <table class="table text-nowrap mb-0 align-middle">
                  ...
                  <tbody>
                    @foreach($book5 as $book5s)
                      <tr>
                        <td class="border-bottom-0"><h6 class="fw-semibold mb-0">{{ $book5s->id }}</h6></td>
                        <td class="border-bottom-0">
                          @if(!empty($book5s->subject))
                            <h6 class="fw-semibold mb-1" style="white-space: pre-wrap;">{{ $book5s->subject }}</h6>
                          @else
                        ...
                        <td class="border-bottom-0">
                          @if(!empty($book5s->text))
                            <p class="mb-0 fw-normal" style="white-space: pre-wrap; width: 250px;">{{ $book5s->text }}</p>
                          @else
                        ...
                        <td class="border-bottom-0">
                          @if(!empty($book5s->image))
                            <img src="{{ asset('storage/images/' . $book5s->image) }}" class="mb-0 fw-normal" style="width: 100px;">
                          @else
                            <p class="mb-0 fw-normal">--</p>
                          @endif
                        </td>
                        <td class="border-bottom-0">
                          @if(!empty($book5s->link))
                            <p class="mb-0 fw-normal link" style="white-space: pre-wrap; width: 250px;">{{ $book5s->link }}</p>
                          @else
                            <p class="mb-0 fw-normal">--</p>
                          @endif
                        </td>
                        <td class="border-bottom-0">
                          <div class="d-flex align-items-center gap-2">
                            <a href="{{ route('edit5',$book5s->id) }}" class="ti ti-pencil fs-6" style="border: none; background-color:transparent;color:green;"></a>
                            <span> | </span>
